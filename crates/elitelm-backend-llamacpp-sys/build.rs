...
fn main() {
    ...
    let cmake_exe = find_cmake().expect(
        "cmake not found. Ensure LLVM is installed and VS CMake integration is present",
    );
    ...
    let ninja = find_ninja().expect(
        "ninja not found. Ensure VS C++ CMake tools are installed.",
    );

    // Add ninja's parent directory to PATH so CMake can find it automatically
    if let Some(ninja_dir) = ninja.parent() {
        let path = std::env::var("PATH").unwrap_or_default();
        let new_path = format!("{};{}", ninja_dir.display(), path);
        unsafe {
    ...
    cfg.define("CMAKE_BUILD_TYPE", "Release")
        ...
        // Point CMake at clang-cl for both C and C++
        .define("CMAKE_C_COMPILER", clang_cl.display().to_string())
    ...
    let bindings = bindgen::Builder::default()
        ...
        .clang_arg(format!("-I{}", llama_src.join("ggml").join("include").display()))
...
fn find_clang_cl() -> Option<PathBuf> {
    ...
    if let Some(vs) = vs_install_path() {
        for sub in ["VC\\Tools\\Llvm\\ARM64\\bin", "VC\\Tools\\Llvm\\bin", "VC\\Tools\\Llvm\\x64\\bin"] {
