...
*   **API Server:** A FastAPI server (`elitelm serve`) to expose the model's functionality as a web API.
...
*   **`session.py`**: This module houses the core runtime logic. The `ChatSession` class manages chat history and streaming generation. It also contains helpers for configuring the QNN execution provider for NPU acceleration.

*   **`cli.py`**: A thin wrapper around `ChatSession` that provides an interactive command-line interface for quick, interactive sessions with the model.
...
```bash
elitelm run configs/my-experiment.yaml
```
